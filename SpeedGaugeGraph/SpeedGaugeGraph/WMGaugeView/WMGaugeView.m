 ...
 * WMGaugeView.h
 *
 * Copyright © 2017 Naresh Kukkala. All rights reserved.
 *
 */
...
- (void)initialize;
{
    ...
    _unitOfMeasurementColor = [UIColor blackColor];
...
//            CGContextStrokePath(context);
            ...
            NSLog(@"attrStr --> %@",valueString);
            CGSize fontWidth;
            fontWidth = [valueString sizeWithAttributes:stringAttrs];
            
            [attrStr drawAtPoint:CGPointMake(0.5 - fontWidth.width / 2.0, y3 - 0.035)];
        }
        // Subdivision
//        else
//        {
...
- (void)drawRangeLabels:(CGContextRef)context
{
    ...
    for (int i = 0; i < _rangeValues.count; i ++)
    {
        ...
        [path addArcWithCenter:center radius:rangeLabelsRect.size.width / 3.0 startAngle:DEGREES_TO_RADIANS(lastStartAngle) endAngle:DEGREES_TO_RADIANS(valueAngle) - 0.01 clockwise:YES];
